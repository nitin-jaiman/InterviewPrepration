package Graph;

import java.util.HashSet;
import java.util.Set;

/**
 * Created by apple on 16/03/17.
 */
public class LongestCycle {

    public int checkCycle(Set<Node> white){


        Set<Node> grey=new HashSet<Node>();
        Set<Node> black=new HashSet<Node>();

        int max=-1;

        while(white.size()>0) {

            Node node = white.iterator().next();
            int x=(DFS(node, white, grey, black));
            if(x!=-1){



                if(max<x){max=x;}

            }

        }

        return max;

    }

    public int DFS(Node node,Set<Node> white,Set<Node> grey,Set<Node> black){

        moveVertex(node,white,grey);

        for(Node n:node.getaList()){


            if(black.contains(n)){continue;}

            if(grey.contains(n)){
              //  System.out.println(n.getData());

                return 1;}

            int x=DFS(n,white,grey,black);

             if(x!=-1){
                 // System.out.println(n.getData());

                return x+1;
            }


        }

        moveVertex(node,grey,black);
        return -1;

    }


    private void moveVertex(Node n,Set<Node> source, Set<Node> destination ){

        source.remove(n);
        destination.add(n);


    }




    public static void main(String args[]){

        Node n5=new Node();
        Node n1=new Node();
        Node n2=new Node();
        Node n3=new Node();
        Node n4=new Node();
        Node n6=new Node();

        n5.getaList().add(n6);

        n5.setData(5);
        n6.setData(6);

        n1.setData(1);
        n1.getaList().add(n3);
        n1.getaList().add(n2);

        n2.setData(2);
        n2.getaList().add(n3);

        n3.setData(3);
        n3.getaList().add(n5);

        n4.setData(4);
        n4.getaList().add(n1);
        n4.getaList().add(n5);

        n6.getaList().add(n4);



        Set<Node> white=new HashSet<Node>();
        white.add(n1);
        white.add(n2);
        white.add(n3);
        white.add(n4);
        white.add(n5);
        white.add(n6);


        LongestCycle longestCycle=new LongestCycle();
        System.out.println(longestCycle.checkCycle(white));


    }


}
